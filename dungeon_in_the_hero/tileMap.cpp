	...
	for (int x = 0; x < m_tileSizeX; x++)
	{
		for (int y = 0; y < m_tileSizeY; y++)
		{
				...
				(tileCheck(x, y))))
			{
				...
				{
				case 1:
					gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "ÀÌ°÷ÀÇ Á¦ ÀÚ¸®±º¿ä?", eTxtBoxColor::Green);
					break;
				case 2:
					gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "±×·³ Â÷³ª ÇÑÀÜ?", eTxtBoxColor::Green);
					break;
				case 3:
					gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "ÈÄÈÄ ¸ÛÃ»ÇÑ ¿ë»çµé!", eTxtBoxColor::Green);
					break;
				case 4:
					gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "¾Æ´ÁÇÑ °÷ÀÌ¿¡¿ä~", eTxtBoxColor::Green);
					break;
				case 5:
					gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "±×·³ ±â´Ù·Áº¼±î¿ä?", eTxtBoxColor::Green);
					break;
				}
...
void tileMap::heroStartSys()
{
	...
	if (m_stageTimer <= 0 && (m_gameState == eGameState::GameReady))
	{
		...
		{
		case 1:
			gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "²¦, ³î·¨ÀÝ¾Æ!", eTxtBoxColor::Red);
			break;
		case 2:
			gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "Àú ¹®ÀÌ ¾ó¸¶Â¥¸°µ¥!", eTxtBoxColor::Red);
			break;
		case 3:
			gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "¿ë»ç°¡ ³ªÅ¸³µ¾î¿ä!!", eTxtBoxColor::Red);
			break;
		case 4:
			gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "±â´Ù¸± ÁÙ ¸ð¸£´Â±¸³ª?", eTxtBoxColor::Red);
			break;
		case 5:
			gameTxtBox(IMAGEMANAGER->findImage("Overlord"), "´ë¸¶¿Õ´ÏÀÌÀÌÀÌ´Ô!!", eTxtBoxColor::Red);
			break;
		}
...
void tileMap::tileSetTxt(int tileType, int tileNum)
{
	...
	case tagTileType::GROUND:
		setTxt.t_txtName = "Àú¼¼»ó ¹Ù´Ú";
		setTxt.t_txtInfo = "´øÀü ¹ÛÀÇ ¹«¼­¿î Èë´õ¹Ì ÀÔ´Ï´Ù!";
		break;
	case tagTileType::START:
		setTxt.t_txtName = "´øÀü ÀÔ±¸";
		setTxt.t_txtInfo = "¿ë»ç°¡ ³ªÅ¸³ª´Â ÀÔ±¸ ÀÔ´Ï´Ù!";
		break;
