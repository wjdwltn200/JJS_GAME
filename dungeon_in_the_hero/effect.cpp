	...
	int fps, float elapsedTime, bool isCam)
{
	...
	m_isCamXY = isCam;
	m_imgSizeX = frameWidth;
	m_imgSizeY = frameHeight;
...
void effect::render(HDC hdc)
{
	...
	if (m_pImg)
	{
		if (m_isCamXY)
		{
			m_pImg->aniRender(hdc, m_nX - CAMERA->getCamPosX(), m_nY - CAMERA->getCamPosY(), m_pAnimation);
		}
		else
		{
...
void effect::startEffect(int x, int y)
{
	...
	m_nX = x + CAMERA->getCamPosX();
	m_nY = y + CAMERA->getCamPosY();
